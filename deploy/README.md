...
공개 dev는 초기 운영 환경으로 사용하므로 `deploy.sh`가 실제 운영자·수탁·보유 정보를
기록한다. 사진은 현재 GCS가 아니라 서울 리전 Compute Engine의 `api-uploads` Docker 영속
볼륨에 저장된다. 운영자 상세 주거 주소는 기록하지 않고 공개 가능한 시·구 단위만 사용한다.
법적 사실이나 저장 방식이 바뀌면 문서 버전과 시행일을 함께 갱신한다.

기존 dev 계정이 모두 출시 전 테스트 계정임을 확인한 경우에만 `deploy.sh`는 v2 동의
migration의 두 확인값을 기록한다. 이 migration은 기존 동의를 `prelaunch_migration` 출처로
전환하고 마케팅 동의 행을 삭제하므로, 외부 사용자 계정이 있는 환경에는 이 배포 경로를 쓰지
않는다.
