...
법적 고지 값은 dev가 외부 사용자에게 제공되지 않는 테스트 환경일 때만 `deploy.sh`가
`개발 환경 테스트용` placeholder를 기록한다. 이 값은 설정 검증과 회원가입 흐름 검증을 위한
임시값이며, 공개 dev 또는 prod 배포 전에는 실제 운영자·수탁·보유 정보로 교체해야 한다.
