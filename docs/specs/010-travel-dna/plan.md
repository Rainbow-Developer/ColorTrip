...
- 카카오 로그인 기능 자체 구현. (단, 로그인된 User 도메인과의 FK 연동 및 `User.dna` 필드 연동은 본 작업 범위에 포함되어 함께 구현됩니다.)
...
## 리스크 / 미해결 질문
- 사용자의 대표 DNA 성향 결과를 `User` 테이블에 직접 캐싱(`User.dna` 컬럼)하여 관리합니다. 이미 마이그레이션(`a4f2c8d1e9b0`)을 통해 `users` 테이블과의 관계 및 FK 제약 조건이 바인딩 완료되었습니다.
