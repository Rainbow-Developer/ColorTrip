...
- 여정 완료 판정(KAN-104 개정): 여행 종료일 다음날 00:00 KST부터(`end_date` < 오늘, KST)
  `completed`다. 종료일 23:59:59 KST까지는 퀘스트 완료 수와 무관하게 `in_progress`다.
- 여정 생성·수정 시 시작일·종료일은 필수이고, 종료일이 시작일보다 빠르거나 이미 지난 기간이면 거부한다.
  같은 사용자의 다른 여정과 날짜 범위가 겹쳐도 허용한다(KAN-108).
...
- `journeys` — 여정. `user_id`(FK→users) · `region_id`(FK→regions) · `title` · `start_date` · `end_date` · `status`(in_progress/completed) · `completed_at`
...
flowchart TD
    ...
    V --> P[("quest_progress")]
    V -->|"완료 판정 재계산"| JDB
    C -->|"여정 조회 (기간 경과 반영)"| L["GET /journeys · /journeys/{id}"] --> JDB
...
| 결정할 항목 | 선택지 | 제안 / 근거 | 상태 |
|------|--------|------------|------|
| 1. 인증·회원 의존 | A) PR #13(feat/005-auth-member) 위에 스택 / B) dev 기준 + 임시 인증 스텁 | **A 확정** — users FK·`CurrentUser`(JWT)가 필요한데 dev에는 없다. PR #13 구현을 참고·사용하고 그 브랜치를 머지해 작업. #13 머지 후 PR diff는 본 작업만 남는다 | 합의됨(2026-07-05) |
...
| 3. 사진 저장소 | A) 로컬 디스크(볼륨) + 추상화로 GCS 후속 / B) 처음부터 GCS | **B 확정** (사용자 결정) — 운영은 GCS 버킷에 업로드. 스토리지 인터페이스는 분리해 로컬 개발·테스트는 로컬 디스크 구현체 사용(버킷 미설정 시). **버킷 생성·SA 권한은 infra(IaC) 후속 작업 필요** | 합의됨(2026-07-05) |
...
| 6. 여정 완료 처리 | A) 마지막 퀘스트 인증 시 자동 완료 / B) 날짜 기반 자동 완료 / C) 명시적 완료 API | **B 확정(KAN-104)** — 퀘스트를 전부 완료했다는 이유만으로 여행이 끝나는 것은 실제 여행 기간과 맞지 않는다. 여정은 종료일 23:59:59 KST까지 진행중이고, 종료일 다음날 00:00 KST부터 완료된다. 완료 퀘스트 수는 진행률에만 쓰고 status 판정에는 쓰지 않는다 | 합의됨(2026-08-22 사용자 확정) |
| 8. 기간 경과 판정 위치 (KAN-73) | A) 여정 조회 시 계산 / B) 스케줄러 배치 | **A 확정**(사용자 결정) — `recalculate_status`는 인증·퀘스트 변경 같은 이벤트에서만 호출되므로 시간이 흐르는 것만으로는 아무도 상태를 갱신하지 못한다. 여정 목록·상세 조회에서 재계산하면 배치·스케줄러 인프라 없이 사용자가 화면을 보는 시점에 반드시 반영된다. 상태가 실제로 바뀔 때만 commit 한다 | 합의됨(2026-08-13) |
| 10. 여정 기간 검증 (KAN-104, KAN-108 개정) | A) 프론트에서만 제한 / B) 백엔드 최종 검증 + 프론트 보조 제한 | **B 확정** — 서버는 필수 기간·종료일 순서·지난 기간만 최종 방어한다. 같은 사용자의 여행 날짜 중복은 사용자가 병행 일정을 만들 수 있도록 허용한다. 프론트는 과거 날짜 선택을 막아 사용자가 빠르게 알아차리게 한다 | 합의됨(2026-08-22), KAN-108 개정(2026-08-27) |
| 9. 사용자 수동 완료(완료 버튼) (KAN-73) | A) `manually_completed_at` 컬럼 + 완료 API / B) status만 단방향 전이 / C) 보류 | **C 확정**(사용자 결정, 2026-08-13) — 이번 범위에서는 자동 판정만 넣고 완료 버튼·API는 만들지 않는다. B는 "퀘스트를 새로 담으면 in_progress로 복귀"(재방문 흐름, KAN-46)를 잃고, A는 컬럼·마이그레이션·API가 함께 필요해 이번 변경 범위보다 크다. 완료 버튼이 다시 필요해지면 A로 재검토 | 보류(후속 이슈) |
...
- **GET /quests/nearby(UX-근처)**: 현재 GPS 좌표 주변 퀘스트를 거리순으로 보여주는 탐색용 API — 여정 플로우에 필수가 아니라 이번 범위에서 제외(2026-07-05 확인). 필요 시 후속.
