...
- 여정 완료 판정(KAN-73 개정): 다음 중 하나라도 만족하면 `completed`다.
  - 여정의 모든 퀘스트가 완료됨, 또는
  - 여행 기간이 지났고(`end_date` < 오늘, KST) 완료 퀘스트가 1개 이상.
  그 밖에는 `in_progress`이며, 완료 조건이 깨지면(예: 퀘스트를 새로 담음) `in_progress`로 되돌린다.
...
| 6. 여정 완료 처리 | A) 마지막 퀘스트 인증 시 자동 완료 / B) 명시적 완료 API | **A 유지 + 조건 확장(KAN-73)** — 최초에는 "모든 퀘스트 완료 = 여정 완료"만 두었으나, 실사용에서 담은 퀘스트를 다 깨지 않고 여행이 끝나는 경우가 대부분이었다. 자동 완료 조건에 **기간 경과(`end_date` < 오늘) + 완료 퀘스트 ≥ 1**을 추가한다. 퀘스트를 나중에 추가하면 in_progress로 되돌리는 규칙은 유지 | 합의됨(2026-08-13 사용자 확정) |
