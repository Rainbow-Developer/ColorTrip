...
- 여정 생성·수정 시 시작일·종료일은 필수이고, 종료일이 시작일보다 빠르거나 이미 지난 기간이면 거부한다.
  같은 사용자의 삭제되지 않은 다른 여정과 날짜 범위가 겹쳐도 거부한다.
...
| 10. 여정 기간 검증 (KAN-104) | A) 프론트에서만 제한 / B) 백엔드 최종 검증 + 프론트 보조 제한 | **B 확정** — 날짜 중복·지난 기간 생성은 데이터 무결성 문제라 서버가 최종 방어한다. 프론트는 과거 날짜 선택을 막아 사용자가 빠르게 알아차리게 한다 | 합의됨(2026-08-22) |
