...
| 2026-08-03 | KAN-59: REC-02 `GET /regions/unvisited` 신설 — 인증 사용자의 `users.dna` 기본 적용, 여정 미생성 지역을 매칭/전체 미완료 퀘스트 수와 함께 페이지네이션 조회(대상 퀘스트는 처음부터 `Quest.client_key IS NOT NULL`로 제한). 기존 REC-01 `GET /quests/recommended`에도 동일한 `client_key IS NOT NULL` 필터를 추가해 Flutter 정적 카탈로그에 없는 퀘스트가 추천·페이지네이션에 섞이지 않게 했다 ([065-quest-recommendation-api](../065-quest-recommendation-api/)) |
