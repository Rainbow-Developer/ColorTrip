...
- **규모 판단**: 한 번에 구현 — 근거: DB 마이그레이션이 없고(`users.profile_image` 컬럼이 이미 존재), `UserProfile` 응답도 그대로다(`profile_image` 필드가 이미 있어 온보딩 요청 body를 건드리지 않는다). 공유 응답에서는 미사용 `owner_profile_image`·`profile_image`를 **제거**하지만([plan.md](./plan.md) 의사결정 5), 소비처가 없어 클라이언트 영향이 없다. 백엔드 endpoint 2개와 프론트 화면 3개가 하나의 사용자 흐름(등록→표시→교체·제거)을 이루므로 쪼개면 어느 단위도 단독으로 검증할 수 없다 — 백엔드만 먼저 넣으면 호출부가 없고, 프론트만 넣으면 저장할 곳이 없다. 총 변경 규모도 신규 파일 3개 + 수정 10여 개 수준이라 한 PR로 리뷰 가능하다.
...
- **스토리지 정리는 best-effort다.** 교체·삭제·탈퇴 시 이전 객체를 지우지만([plan.md](./plan.md) 의사결정 2), 삭제 호출이 실패하면 로그만 남기고 고아 객체로 둔다 — 사용자에게 성공한 작업을 실패로 되돌리지 않기 위해서다. 잔여 객체를 주기적으로 걷어내는 lifecycle 규칙은 여전히 후속 과제다.
- **가입 중단 시 이미지는 계정에 남는다.** 선택 즉시 업로드 방식이라 온보딩을 끝내지 않고 이탈해도 `profile_image`는 유지된다. 같은 카카오 계정으로 재로그인하면 그대로 보이며, 회원가입 중단 안내 문구에 이 동작을 명시했다.
- **이미지 원본이 URL만 알면 공개 접근된다.** GCS 공개 읽기 버킷 전제([010-journey](../010-journey/) 의사결정 3)를 그대로 따른다. 삭제·탈퇴 시 객체를 지우므로 사후 노출은 막히지만, 유효한 이미지의 URL이 유출되면 누구나 볼 수 있다. 비공개 버킷 + signed URL 전환은 이 스펙 범위 밖이다.
- **운영 스토리지가 아직 확정되지 않았다.** dev 배포에 `GCS_UPLOAD_BUCKET`도 볼륨도 없어 로컬 디스크로 폴백하며, 재배포마다 파일이 사라진다([description.md](./description.md) 참고). 출시 전 필수 인프라 작업으로 별도 처리한다.
