...
백엔드의 `UserProfile` 응답은 그대로 둔다. `profile_image`가 이미 있어 온보딩 요청 body(`extra="forbid"`)를 건드릴 필요가 없다. 다만 공유 응답에서는 미사용 아바타 필드를 **제거**한다(의사결정 5) — 소비처가 없어 클라이언트 영향은 없다.
...
| 2. 교체·삭제·탈퇴 시 이전 스토리지 객체 정리 | A) 즉시 삭제<br>B) v1에서는 방치하고 후속 과제로 | **A로 확정 (2026-08-13 코드 리뷰 반영, 최초 제안 B에서 번복).**<br><br>최초에는 B를 제안했다 — URL→object_name 역매핑에 **카카오 CDN URL 제외**가 필요하고 매핑 버그는 복구 불가능한 삭제로 이어지며, 퀘스트 사진도 정리하지 않아 방치가 일관적이라는 이유였다.<br><br>번복 근거: 스토리지가 **공개 읽기 전제**라 컬럼만 비우면 **이전 URL을 아는 주체가 삭제·탈퇴 후에도 이미지를 계속 읽을 수 있다.** 개인정보처리방침의 "탈퇴 시 지체 없이 파기"와 정면으로 어긋나는 동작이라, 후속 과제로 미룰 성질이 아니다.<br><br>안전장치: 소유 판별을 `PhotoStorage.owned_object_name()`으로 스토리지 구현에 위임해 우리가 저장하지 않은 URL(카카오 CDN 등)은 애초에 후보에서 빠지고, `..`가 섞인 경로도 거부한다. 삭제는 **DB 커밋 이후 best-effort**로 수행해 실패해도 사용자 작업을 되돌리지 않고 로그만 남긴다 | 합의됨 |
...
| 7. `privacy-v1` 버전 상향 | A) 유지<br>B) `privacy-v2`로 상향 | **A 유지 (2026-08-13 코드 리뷰에서 재확인).** 수집 항목이 늘어 개인정보처리방침 본문은 갱신하지만, 아직 Play Console 등록 전이라 재동의를 받아야 할 운영 사용자가 없다. B로 올리면 `has_current_required_consents()`가 전원 실패해 **기존 사용자 전체가 `onboarding_step="profile"`로 되돌아간다** — 얻는 것 없이 사용자를 온보딩으로 되돌리는 비용만 든다.<br><br>**전제와 출시 게이트**: 이 결정은 "출시 시점에 배포되는 방침 본문 = `privacy-v1`로 동의받은 본문"이라는 전제 위에 있다. 아래 조건 중 하나라도 깨지면 상향해야 한다.<br>① Play Console 등록·출시 이후 수집 항목이 바뀔 때<br>② 출시 전이라도 실제 사용자(내부 테스터 제외)가 `privacy-v1`에 동의한 이력이 있을 때<br>버전 상수의 SOT는 [auth-security.md](../../conventions/auth-security.md)의 동의 버전 행이다 | 합의됨 |
