...
| 환경 변수 | 기본값 | 설명 |
|-----------|--------|------|
| `GCS_UPLOAD_BUCKET` | (빈 값) | **개발·테스트에서만 생략한다.** 설정하면 GCS에 저장하고, 비어 있으면 로컬 디스크로 폴백한다 |
| `UPLOAD_DIR` | `./uploads` | 로컬 스토리지 경로 (폴백 전용) |
| `MAX_UPLOAD_SIZE_MB` | `10` | 서버 측 업로드 용량 상한 |

> **운영 배포 주의**: 현재 dev 배포(`deploy/docker-compose.yml`)는 `GCS_UPLOAD_BUCKET`도 볼륨 마운트도 없어 로컬 디스크로 폴백한다. 컨테이너 재생성마다 업로드 파일이 사라지고 DB의 URL만 남아 깨진 이미지가 된다. 다중 인스턴스로 확장하면 인스턴스마다 결과가 달라진다. **GCS 버킷 주입 또는 영속 볼륨 부착은 출시 전 필수 조건**이며, 이 스펙 범위 밖의 별도 인프라 작업이다. 프로필 이미지가 생기기 전부터 퀘스트 인증 사진에도 있던 문제다.

허용 형식은 `image/jpeg`, `image/png`, `image/webp`, `image/heic`다. **선언한 content type과 실제 바이트의 시그니처가 일치하는지 검증**하므로, 헤더만 이미지로 위장한 파일은 저장되지 않는다. 클라이언트는 업로드 전에 5MB로 한 번 더 제한한다.
