...
| 2026-08-15 | **배경 지도 URL 식별자 수정(KAN-91, 사용자 보고 "지도 이미지가 전혀 안 뜬다")** — KAN-90 배포 후에도 배경이 안 나왔다. 서버·배포·키는 모두 정상이었고(실제 퀘스트 UUID로 호출하면 `200 image/png 116KB`), **앱이 만든 URL만 틀렸다.** 앱의 `quest.id`는 client_key(`dy3`)인데 서버 경로는 UUID를 받는다 — 다른 API 호출은 전부 `DomainCatalog.questId(key)`로 변환하는데 지도만 그 규칙에서 빠졌다. 서버가 거절하고 `Image.network`의 errorBuilder가 격자 도식으로 내려앉아 **fail-soft가 원인을 가렸다**(화면은 멀쩡해 보이고 배경만 없었다). `_mapImageUrl()`이 카탈로그로 UUID를 찾도록 고쳤고, 매핑이 없으면 null을 넘겨 배경 없이 그린다. 테스트 2건 — URL이 UUID를 쓰고 client_key를 쓰지 않는지, 매핑이 없어도 인증 버튼이 살아 있는지. 수정을 되돌리면 실패하는 것까지 확인했다. 교훈: fail-soft는 사고를 막지만 **조용히 막기 때문에**, 폴백 경로에는 "왜 폴백했는지"가 드러나는 검증이 함께 있어야 한다 |
