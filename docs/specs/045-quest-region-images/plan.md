...
| 결정할 항목 | 선택지 | 제안 / 근거 | 상태 |
|------|--------|------------|------|
| 이미지 소스 | TourAPI CDN 핫링크 / 에셋 번들 / 자체 스토리지 | **TourAPI CDN**(공공데이터 제공 URL)을 기본으로 사용한다. 단, KAN-100 지역 개요 브랜딩 대표 이미지 11개는 로컬 asset을 사용한다. 전체 퀘스트 이미지를 에셋 번들로 전환하는 것은 용량·수집 비용 때문에 비목표로 유지한다. | 합의됨(구현 승인, KAN-100 지역 대표 이미지 예외 추가) |
