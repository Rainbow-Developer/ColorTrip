...
| 결정할 항목 | 선택지 | 제안 / 근거 | 상태 |
|------|--------|------------|------|
| 이미지 소스 | TourAPI CDN 핫링크 / 에셋 번들 / 자체 스토리지 | **TourAPI CDN**(공공데이터 제공 URL). 에셋 번들은 220장 용량·수집 비용 과다, 자체 스토리지는 오버엔지니어링. TourAPI 이미지는 공공누리 제공 콘텐츠로 서비스 표기 규약만 지키면 사용 가능 | 합의됨(구현 승인) |
