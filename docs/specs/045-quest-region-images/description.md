...
퀘스트와 지역에 한국관광공사 TourAPI의 대표 이미지(`firstimage`)를 연결해, 앱의 회색 placeholder를 실제 관광지 사진으로 바꾼다. 이미지 URL과 좌표는 보강 스크립트가 정적 데이터(dart)에 채워 넣고, 화면은 공용 이미지 위젯으로 표시한다. 단양군처럼 별도 제작한 대표 이미지가 있는 지역은 `asset:` 스킴으로 로컬 asset을 지정할 수 있다.
...
2. 화면은 `AppNetworkImage`로 이미지를 로드한다. 원격 URL은 `cached_network_image`로 캐시하고, `asset:assets/...` 값은 Flutter asset으로 렌더링한다. 로딩 중·실패·URL 없음 → 기존 placeholder 색 박스 유지.
...
| 공용 이미지 위젯 | 원격 이미지 캐시·로컬 asset 렌더링·로딩/에러 폴백 | `frontend/lib/core/widgets/app_network_image.dart` |
...
`Region(id: 'danyang', ... imageUrl: 'asset:assets/images/region_danyang_main.png')`
