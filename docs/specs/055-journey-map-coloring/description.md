...
충북 11개 시·군 지도의 채색 진하기를 "그 지역에서 **완료한 여행(여정) 수**"로 결정한다. 여행을 한 번 완주하면 지역이 칠해지고, 같은 지역을 여러 번 여행할수록 진해진다(5회에 최대 채도).
...
3. 채도 = `완료 여행 수 / 5` (0.0~1.0 클램프) → `ChungbukMap`이 회색→진녹색으로 연속 보간해 그린다.
...
- 청주시 여행 1회 완료 → 채도 20%(1단계) / 5회 이상 → 100%(5단계).
