...
| 상태 | 구현 완료 (2026-07-31) / KAN-73 채색 단위 재정의 진행 중 |
...
- [x] (KAN-73) 범례 문구 확인 — `map_legend.dart`는 "여행 완료 횟수"를 쓰지만 새 기준에서는 "인증한 여행 수"에 가깝다. 문구 변경은 이번 범위에서 보류(라벨만 바꾸면 5단계 스와치 설명과 함께 손봐야 해 별도 판단 필요)

## 리스크 / 미해결 질문

- 로컬 누적값은 메모리 상태라 앱 재시작 시 초기화된다(서버 동기화 값은 재조회). 영속화는 후속 과제.
- (해소) "로컬 단독은 0/1로 제한" 제약은 완주 시점 누적 방식으로 바꿔 없어졌다 — 같은 지역 재방문 완주도 2회로 센다.
- (KAN-73) 범례 라벨이 "여행 완료 횟수"로 되어 있어, 새 기준에서는 "인증한 여행 수"에 가깝다. 문구 조정 여부는 구현 중 화면을 보고 판단한다.
