...
flowchart TD
    ...
    travel -->|여행 탭| journey[여행 상세: 선택 퀘스트]
    overview -->|새 여행 만들기| select[퀘스트 선택: 다중 선택]
    overview -->|추천 퀘스트 탭| detail[퀘스트 상세: 정보만]
    overview -->|이 지역의 내 여행 선택| journey
    journey -->|미완료 퀘스트 탭| verify[인증: 사진/GPS/OX퀴즈]
    journey -->|퀘스트 수정하기| select
    select -->|여행 시작하기| travel
    select -->|"퀘스트 설명"| detail
    verify -->|사진 인증 완료| photoResult[사진 검증 결과: AI 지표+완료 안내]
    verify -->|"GPS/OX퀴즈/QR 완료: 직전 화면 복귀"| journey
...
**지역 탐색 → 여행 상세 흐름(KAN-99)**: "여행"은 지역 퀘스트 완료 현황이 아니라 사용자가 지역에서 선택한 퀘스트 묶음이다. 지도에서 지역을 탭하면 **"여행하기"(지역 개요) 화면**이 뜨고, 특정 여행을 탭하면 독립된 여행 상세 화면으로 이동한다.
- **지역 개요**: 지역 이미지, 아이콘·이름·설명만 남긴 간결한 DNA 카드, 추천 퀘스트 3건의 스냅형 가로 캐러셀, **"이 지역의 내 여행"** 목록, **"새 여행 만들기"** 버튼을 순서대로 표시한다. 추천 카드는 탭하면 정보 전용 퀘스트 상세로 이동하고, 진행중 여행만 지역별로 표시한다.
- **새 여행 만들기** → **"퀘스트" 선택 화면**(검색·유형 필터, 카드 탭으로 **다중 선택**, 카드의 "퀘스트 설명"으로 퀘스트 상세 진입). "여행 시작하기"를 누르면 선택 퀘스트를 그 자리에서 수행하지 않고 새 여행으로 등록한 뒤 **여행 탭**으로 이동한다.
- **여행 상세(`/journey/:journeyId`)**: 선택한 여행의 이름·기간·진행률과 퀘스트 목록에 집중한다. 미완료 퀘스트는 상세를 거치지 않고 곧바로 인증 화면으로 이동하고, 완료 퀘스트는 정보 상세로 이동한다. 진행중 여행은 **"퀘스트 수정하기"**로 기존 선택 화면을 재사용하며 저장 후 같은 여행 상세로 돌아온다. 완료 여행은 읽기 전용이다.
- **퀘스트 상세 화면은 어디서 들어가든 순수 정보 화면**이다 — 인증/수행 버튼이 없다(추천 퀘스트 탭, "퀘스트 설명" 링크, `/quests` 전체 목록 어디서 들어가도 동일).
- 여행 목록의 **진행중/지난** 분류는 서버 여정 상태(`Journey.status`)를 그대로 쓴다 — 진행중 = `in_progress`, 지난 = `completed`. 완료 판정 규칙(종료일 다음날 00:00 KST부터 완료)의 SOT는 [010-journey](../010-journey/description.md#여정-완료-판정)다.
...
| 여행 목록·상세 | 진행중/지난 여행 목록, 여행별 퀘스트 상세·수정 진입 | `frontend/lib/features/travel/` |
| 퀘스트 | 지역 개요("여행하기")·퀘스트 선택(검색·필터)·정보 상세·인증(사진/GPS/퀴즈), 유형별 전체 목록(보조) | `frontend/lib/features/quests/` |
