...
- 여행 목록의 **진행중/지난** 분류도 이 여행(선택한 퀘스트 묶음) 기준으로 바뀌었다 — 진행중 = 여행 시작함 + 선택한 퀘스트 중 미완료 있음, 지난 = 여행 시작함 + 선택한 퀘스트 전부 완료. (홈의 "완료 지역"·"진행률" 스탯과 지도 색칠은 기존처럼 지역별 퀘스트 완료 개수 기준을 그대로 유지 — 별개 지표.)

**핵심 인터랙션**
- **여행 DNA**: 4문항 설문에서 각 선택지가 5개 유형(자연/미식/역사/액티비티/힐링) 중 하나에 매핑되고, 최다 선택 유형이 DNA가 된다.
- **퀘스트 인증**: 사진(업로드→**사진 검증 결과 화면**(AI 지표·완료 안내)→"지도에서 확인하기"로 홈 이동)·GPS(위치 확인 UI→즉시 완료 후 이전 화면들로 pop)·OX퀴즈(정답 시 완료). 완료하면 `completed`/지역 `progress`/`timeline`/리워드 포인트가 갱신된다.
- **지도 색칠**: 지역 진행도(완료 퀘스트 개수 ÷ 6, 0.0~1.0로 클램프 — 6개 이상 완료하면 100%)에 비례해 회색(`#CCCCCC`)에서 진초록(`primaryDark` `#2D6A4F`)까지 단일 색조로 연속 채도 보간한다(2026-07-21 — 기존 3단계(0/1/2+, 2+는 파랑)에서 변경, KAN-44). 지역마다 정적 퀘스트 개수가 제각각이라(1~3개) 지역 전체 개수 대비 비율로 계산하면 퀘스트가 적은 지역만 쉽게 100%가 돼버려, 모든 지역에 같은 기준선(6개)을 쓴다 — `completed_count`는 재방문 시 계속 늘어날 수 있어 정적 개수를 넘어설 수 있다(KAN-46). 퀘스트 완료 시 로컬에서 즉시 +1 반영(낙관적 갱신)하고, 앱 진입 시 백엔드(`GET /users/me/map`)의 `completed_count`로 덮어써 동기화한다([020-frontend-map-sync]).
