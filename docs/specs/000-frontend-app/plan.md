...
> [035 Kakao 통합 인증](../035-kakao-auth-integration/)을 따른다. 여행·퀘스트·지도·타임라인의
> 메모리 상태는 [040 서버 영속화](../040-domain-state-persistence/)에서 교체할 계획이다.
...
**상태(초기 프로토타입 결정)**: 앱 전역 진행 상태(완료 퀘스트·지역 진행도·타임라인·DNA·설문 진행)를
Riverpod 메모리로 관리했다. 이 결정은 백엔드가 없던 초기 범위의 기록이며, 인증 상태는 035가,
여행·퀘스트·지도·타임라인 서버 상태는 040이 각각 대체한다.
...
| 결정할 항목 | 선택지 | 제안 / 근거 | 상태 |
|------|--------|------------|------|
| 데이터·상태 소스 | A) 정적 mock+메모리 / B) 정적 mock+로컬 영속 / C) API 계약 먼저 | **A 채택(초기 프로토타입).** 당시 백엔드가 없어 메모리 구현을 선택했다. 현재 백엔드가 준비되어 040에서 정적 표시 카탈로그는 유지하고 사용자 도메인 상태를 서버 SOT로 전환한다. | 합의됨 / 040이 후속 대체 |
