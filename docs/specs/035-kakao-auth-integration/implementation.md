...
| 2026-08-15 | **생년월일 휠 조작 결함 수정(KAN-89, 사용자 보고)** — ① 월 휠만 굴려도 한 칸씩밖에 안 움직였다. 월 변경 콜백이 `_syncMonthAndDay()`를 통해 **자기 컨트롤러**에 `jumpToItem`을 걸어 스크롤 관성을 매 칸 끊고 있었다. 월이 바뀔 때 실제로 다시 잡아야 하는 것은 일 범위뿐이므로(월 범위는 연도에만 의존) `_syncForYearChange`(월+일)와 `_syncForMonthChange`(일만)로 나눴다. 연·일 휠에 증상이 없던 이유도 그쪽은 자기 컨트롤러를 되돌리지 않기 때문이다. ② 선택된 값이 보이지 않았다 — `selectionOverlay`가 불투명한 `surfaceMuted`로 채워져 있는데 `CupertinoPicker`는 이 오버레이를 아이템 **위에** 겹쳐 그린다. 글자가 비치도록 옅은 배경 + 테두리로 바꿨다. 테스트 2건 추가(월 다단 이동 — 수정 전 실패 확인, 월 이동 시 일 보정 유지) |
