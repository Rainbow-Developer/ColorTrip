...
3. 외부 사용자가 그 링크를 열면 백엔드의 `GET /share/{share_code}`가 기존 `GET /api/v1/shares/{share_code}`와 동일한 데이터(진행률·DNA·색칠 지역, 공유 스타일별 필터링)를 사람이 읽는 HTML 카드로 렌더링한다. `MAP`/`MAP_AND_DNA` 스타일은 색칠 지역이 0개여도 앱 홈 화면 지도와 같은 11개 시·군 SVG path 지도를 보여주고, `DNA`/`MAP_AND_DNA` 스타일은 DNA 유형명과 설명을 보여준다. 하단에는 "앱에서 열기"(`colortrip://` 커스텀 스킴, 설치돼 있으면 앱 실행 후 기존 로그인 상태 기반 라우팅에 맡김)와 "앱 다운받기"(Play 스토어 링크, placeholder) 버튼을 제공한다. 앱은 `colortrip://share/{code}`가 `/share/{code}` 또는 `/{code}`로 들어와도 not found가 아니라 공유 화면으로 열고, 공유 코드는 인앱 콘텐츠 조회에 사용하지 않는다.
...
* "OO님의 여행 진행률 45%"
* 공개용 충북 SVG 지도(색칠 지역이 0개여도 11개 시·군 path와 라벨 표시)
