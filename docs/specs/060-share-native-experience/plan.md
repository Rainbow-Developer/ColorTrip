...
| 결정할 항목 | 선택지 | 제안 / 근거 | 상태 |
|------|--------|------------|------|
| 랜딩 페이지에 실제 지도 이미지 포함 여부 | A) 클라이언트가 지도를 PNG로 캡처해 업로드, 랜딩에 표시<br>B) 백엔드 HTML 전용 11개 시·군 지도 그리드 표시(이미지 업로드 없음) | **B 선택** — 이미지 캡처·업로드·스토리지(GCS/로컬) 인프라를 새로 만들지 않고, 공개 링크에서 지도 맥락이 사라지지 않도록 0/11 상태에도 지도 그리드를 유지한다. | 합의됨 |
