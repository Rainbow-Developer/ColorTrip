...
| PR 전략 | 선행 PR 병합 대기 / 의존성을 밝힌 Draft 선등록 | **Draft 선등록**. 현행 규칙대로 `dev`를 대상으로 하되 #48·#49 의존성과 임시 중복 diff를 본문에 명시한다. 두 PR 병합 후 KAN-55 전용 커밋만 최신 `dev`로 옮기고 분할 차이를 재검토한 뒤 Ready로 전환한다. | 합의 반영 |
...
- Draft 단계의 diff에는 아직 `dev`에 없는 #48·#49 변경과 닫힌 #47 기반의 일부 차이가
  함께 보인다. 이를 KAN-55 최종 변경으로 오인하지 않도록 PR 본문과 리뷰 안내에
  의존성을 명시하고, 두 선행 PR 병합 후 반드시 최신 `dev` 기준으로 다시 산정한다.
