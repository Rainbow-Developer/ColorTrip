...
- [~] 8) 브랜치 마무리 — #48·#49 병합 후 최신 `dev`에 재배치했고, KAN-62의
  Trip DNA 접근 제어도 포함했다. PR #50 코드리뷰 반영과 재검증을 진행한다.
...
- 여정 목록의 `quest_client_keys` bulk 응답으로 앱 부팅 시 여정별 상세 N+1 제거,
  여행 카드에서 선택한 `journeyId`를 여행 상세·선택·인증 화면까지 전달
...
| 2026-08-01 | PR #50 리뷰 반영: 업로드 사진 사용자 소유권, 타임라인 멱등성, Flutter 복원·인증 화면 안정성 보강 |
| 2026-08-19 | KAN-99에서 기존 여정 스냅샷을 지역별 진행중 여행과 독립 여행 상세 화면에 재사용하도록 Flutter 표시 흐름을 변경(API·DB 계약 변경 없음) |
